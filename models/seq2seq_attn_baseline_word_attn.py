...
class encoder(nn.Module):
    def __init__(
        ...
        logger.info('Not using embedding dropout')
    ...
    def forward(self, input, input_hashes, lengths = None, hidden = None):
        ...
        return hidden_t, outputs, None, emb # self.emb_dropout(emb)
...
class decoderBase(nn.Module):
    ...
    def forward(
        ...
        hidden, outputs, attns, coverage = self._run_forward_pass(input, input_charngrams, context, encoder_embedding, state, context_lengths = context_lengths)
...
class Seq2SeqAttention(nn.Module):
    ...
    def forward(self, batch):
        ...
        enc_hidden, context, penalty, emb_outputs = self.encoder(src, None, src_lens)
        encoderOutputs = EncoderOutputs(enc_hidden, context, emb_outputs)

        enc_state = self.decoder.init_decoder_state(src, context, enc_hidden)

        out, dec_state, attns, predictions = self.decoder(tgt, None, encoderOutputs.outputs, emb_outputs, enc_state, context_lengths=src_lens)
        decoderOutputs = DecoderOutputs(predictions, None, out, dec_state, attns=attns)
